#!/usr/bin/env python3
"""Independent checker for the x402 settlement-receipt binding over a metered
EV-charge lifecycle (the `sui` rail, `upto` scheme).

Vendored verbatim from the pinned vaaraio/vaara v1.1.1 conformance checker
(tests/vectors/x402_settlement_v0/_check_independent.py); the only change is
``_RAILS`` pointed at the sui charge vector. The verdict logic is untouched, so
"passes" here means the same thing it means upstream.

For each step it reproduces three verdicts from settlement + receipt alone:

  action_ref_recomputes        sha256 over the JCS-canonical action tuple
                               (agentId, actionType, scope, timestampMs, seq,
                               terminal) equals settlement.actionRef. Nothing
                               rail-specific (and not the amount) enters the tuple.
...
And one cross-step verdict:
  ...
  lifecycle_distinguishes_terminal
                               the authorized-ceiling voucher (step0) and the
                               finalized settlement (step1) have distinct
                               action_refs, carry terminal=false / terminal=true,
                               and the voucher receipt does not resolve against the
                               finalized settlement.

Run: python vectors/_check_independent.py
